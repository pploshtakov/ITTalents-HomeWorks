...
public class Driver {
	...
	public void getTruckWithExpiresVignette(String expiresDate) {
		for (int i = 0; i < vehicles.length; i++) {
			if (vehicles[i].getType().equals(Vehicle.TYPE_TRUCK)
					&& vehicles[i].getVignette().getExpiryDate().equals(expiresDate)) {
				System.out.println(this.name + " has a truck with expires vignette of " + expiresDate);
			}
		}
	}
